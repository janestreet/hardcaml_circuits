...
(** Create the update logic for a lfsr. Used in conjuction with [reg_fb] to construct a
    complete [lfsr].
    ...
    - The basic gate can be [xor] or [xnor]. With [xor] the all 0's state is invalid while
      with [xnor] the all 1's state is invalid.
    ...
    - All complete lfsr have a counterpart organisation of the taps which leads to a
      second (but still complete) sequence. *)
val create
