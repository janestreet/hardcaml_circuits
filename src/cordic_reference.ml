...
let iter ~iterations ~init ~f =
  let rec loop i ih k acc =
    ...
    then acc
      ...
      if ih = k then loop (i + 1) ih ((k * 3) + 1) acc else loop (i + 1) (ih + 1) k acc)
  in
  loop 0 1 4 init
;;

let gain ~iterations =
  iter ~iterations ~init:1. ~f:(fun ~i ~ih:_ a ->
    a *. Float.(sqrt (1. +. (2. ** (2. *. -of_int i)))))
;;

let gainh ~iterations =
  iter ~iterations ~init:1. ~f:(fun ~i:_ ~ih a ->
    a *. Float.(sqrt (1. -. (2. ** (2. *. -of_int ih)))))
;;
...
let cos_sin ~iterations angle =
  ...
  let x, y, _ =
    cordic () ~system:Circular ~mode:Rotation ~iterations ~x:(1. /. gain) ~y:0. ~z:angle
  in
  x, y
;;
...
let polar_to_rect ~iterations mag phase =
  let gain = gain ~iterations in
  let x, y, _ =
    cordic () ~system:Circular ~mode:Rotation ~iterations ~x:(mag /. gain) ~y:0. ~z:phase
  in
  x, y
;;
...
let rotate_vector ~iterations x y angle =
  let gain = gain ~iterations in
  let x, y, _ = cordic () ~system:Circular ~mode:Rotation ~iterations ~x ~y ~z:angle in
  x /. gain, y /. gain
;;
...
let atan ~iterations a =
  let _, _, z =
    cordic () ~system:Circular ~mode:Vectoring ~iterations ~x:1.0 ~y:a ~z:0.
  in
  z
;;

let atan2 ~iterations y x =
  let _, _, z = cordic () ~system:Circular ~mode:Vectoring ~iterations ~x ~y ~z:0. in
  z
;;

let rect_to_polar ~iterations x y =
  let gain = gain ~iterations in
  let x, _, z = cordic () ~system:Circular ~mode:Vectoring ~iterations ~x ~y ~z:0. in
  x /. gain, z
;;
...
let mul ~iterations a b =
  let _, y, _ = cordic () ~system:Linear ~mode:Rotation ~iterations ~x:a ~y:0. ~z:b in
  y
;;

let div ~iterations a b =
  let _, _, z = cordic () ~system:Linear ~mode:Vectoring ~iterations ~x:b ~y:a ~z:0. in
  z
;;
...
let cosh_sinh ~iterations a =
  ...
  let x, y, _ =
    cordic () ~system:Hyperbolic ~mode:Rotation ~iterations ~x:(1. /. gainh) ~y:0. ~z:a
  in
  x, y
;;
