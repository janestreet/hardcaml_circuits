(** A counter. If the current value is [n], it outputs [n]/[divisor]
    (as [quotient]) and [n] mod [divisor] (as [remainder]). Since the counter doesn't
    track [n] directly, the maximum values must be specified as [max_quotient] and
    [max_remainder], and the counter will wrap when its outputs reach [max_quotient] AND
    [max_remainder].
