...
let test_range_2_1 ~start ~stop ~steps ~f_cordic ~f =
  test_range_2' ~start ~stop ~steps ~f_cordic ~f ~error
;;
...
let%expect_test "atan2, no particular convergence range" =
  let errors =
    test_range_2_1
      ...
      ~stop:2.
      ~steps:4
      ~f_cordic:(Cordic_reference.atan2 ~iterations:16)
      ~f:Float.atan2
  in
...
module Polar = struct
  ...
  let to_rect { magnitude; phase } : Rect0.t =
    { x = magnitude *. Float.cos phase; y = magnitude *. Float.sin phase }
  ;;
end
...
module Rect = struct
  ...
  let to_polar { x; y } =
    { magnitude = Float.(sqrt ((x * x) + (y * y))); phase = Float.(atan (y / x)) }
  ;;
end
...
(* [|a| < 3.4], [|b| < 2.] *)
let%expect_test "mul" =
  let mul a b = error (Cordic_reference.mul ~iterations:16 a b) (a *. b) |> Float.log10 in
...
let%expect_test "div" =
  let div a b = error (Cordic_reference.div ~iterations:16 a b) (a /. b) |> Float.log10 in
